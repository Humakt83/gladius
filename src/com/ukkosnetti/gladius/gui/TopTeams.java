...
public class TopTeams extends JFrame {
	...
	public TopTeams(Vector<Team> t) {
		...
		while (it.hasNext()) {
			Team apu = it.next();
			// First if clause is checking if the Team has a place in Top10 thus
			// we compare it to last entry in array.
			if (apu.getChampionWins() > Integer.parseInt(top10[9][2]) || (apu.getChampionWins() == Integer.parseInt(top10[9][2]) && (apu.getMatchWinsAllTime() > Integer.parseInt(top10[9][1])))
					|| Integer.parseInt(top10[9][2]) == 0) {
				// Going the array through using loop in order to place the team
				// entry into proper place while moving others.
				for (int i = 0; i < 10; i++) {
