...
public class BattlePanel extends GradientPanel implements Runnable {
	...
	private boolean missOrSplat = false; // Used for showing either miss or hit
											// image.
	private int hitx = 0, hity = 0; // Where on the battletable (Battle) e was
									// hit made.
	...
	// Thread which purpose is to make miss or hit image to disappear after 400
	// milliseconds.
	public void run() {
		while (running) {
			this.repaint();
	...
	public void paintComponent(Graphics g) {
		...
		if (battlestarted) {
			for (int i = 0; i < 5; i++)
				g.drawImage(tree.getImage(), treetable[i][0] * 40, treetable[i][1] * 40, null);
			for (int i = 0; i < 5; i++)
				g.drawImage(rock.getImage(), rocktable[i][0] * 40, rocktable[i][1] * 40, null);
			for (int x = 0; x < 10; x++) {
				for (int y = 0; y < 8; y++) {
