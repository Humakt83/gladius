...
public class DAO{
	...
	public DAO() {     
     ...
     public Vector<Spell> searchSpells(boolean damage){
		...
		try{
            String sqlSelect;
            if(damage)sqlSelect = "SELECT * FROM Spell WHERE damagespell";
            else sqlSelect = "SELECT * FROM Spell WHERE NOT damagespell";
	...
	public Vector<Gladiator> searchGladiators(String Team, Game g){
		...
		try{
			...
			while (myRs.next()) {
                ...
                if(meleestr == null || meleestr.isEmpty()) melee = null;
                else melee = g.getWeaponBlacksmith(meleestr);
                String rangedstr = myRs.getString("ranged");
                if(rangedstr == null || rangedstr.isEmpty()) ranged = null;
                else ranged = g.getWeaponBlacksmith(rangedstr);
                String spell1str = myRs.getString("spell1");
                if(spell1str == null || spell1str.isEmpty()) spell1 = null;
                else spell1 = g.getOneSpell(spell1str);
                String spell2str = myRs.getString("spell2");
                if(spell2str == null || spell2str.isEmpty()) spell2 = null;
                else spell2 = g.getOneSpell(spell2str);
                String armorstr = myRs.getString("armor");
                if(armorstr == null || armorstr.isEmpty()) armor = null;
                else armor = g.getOneArmor(armorstr);
                ...
                gladiators.add(new Gladiator(damagemin, damagemax, race, naturalarmor,
                    maxhealth, attack, defense, agility,
                    resistance, swordatt, sworddef, spearatt, speardef, axeatt, axedef,
                    hammeratt, hammerdef, bowskill, crossbowskill,
                    maxmana, name, upkeep, picture, strength, evasion));
