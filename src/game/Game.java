...
public class Game implements Serializable{
    ...
    public boolean canHire(){
        if(activeTeam.getMembers()>=6)return false;
        else return true;
    }
    ...
    public void purchaseItem(String n, View v){
        ...
        if(!success) success = spellshop.purchase(activeTeam, n, currentGladiator, v);
        if(success){
            v.setSquirrels(this.getActiveSquirrels());
            v.showGladiator(currentGladiator);
        }
        else v.addText("Get out of here you beggar!");
    }
    public boolean checkIfBeholder(){
    	if(currentGladiator.getRace().equals("Beholder")||currentGladiator.getRace().equals("Beholder_Hero"))return true;
    	else return false;
    }
    public Vector<Gladiator> getCurrentGladiators(boolean change){
        if(activeTeam.getGladiators()!=null&&change)currentGladiator = activeTeam.getGladiators().elementAt(0);
    ...
    public Vector<Spell> getSpells(boolean d){
        if(d)return spellshop.getDamageSpells();
        else return spellshop.getHealingSpells();
    }
    public Vector<Team> getTeams(){
    	return teams;
    }
    public void newSeason(View v){
        String league1high = null, league2high = null, league3high = null,
                league4high = null, league1low = null, league2low = null,
                league3low = null;
        int league1highint = -1, league2highint = -1, league3highint = -1,
                league4highint = -1, league1lowint = -1, league2lowint = -1,
                league3lowint = -1;
        ...
        v.addText("Following teams have risen up in the leagueladder: " + league2high
                + ", " + league3high + " and " +league4high +". All ladder-risers earn 500 bonus squirrels!" );
        v.addText("While following teams have succumbed to lower league: " + league1low
                + ", " + league2low + " and " +league3low +"." );
